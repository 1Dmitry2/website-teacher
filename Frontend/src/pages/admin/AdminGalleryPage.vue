  ...
  <div class="max-w-7xl mx-auto px-4 py-10 space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p class="text-gray-500 mt-1">Управление фотогалереей</p>
      </div>
      <ui-button type="button" variant="primary" @click="showCreateModal = true">Добавить фото</ui-button>
    </div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-3 lg:grid-cols-4">
      <div v-for="item in gallery" :key="item.id" class="bg-white rounded-xl shadow overflow-hidden group">
        <img :src="item.image_url" :alt="item.title || 'Image'" class="w-full h-48 object-cover" />
        <div class="p-4">
          <h3 class="font-semibold text-gray-900 truncate">{{ item.title || 'Без названия' }}</h3>
          <p class="text-sm text-gray-500 truncate">{{ item.description }}</p>
          <div class="mt-2 flex gap-2">
            <button @click="editItem(item)" class="text-xs text-indigo-600 hover:text-indigo-900">Редактировать</button>
            <button @click="deleteItem(item.id)" class="text-xs text-red-600 hover:text-red-900">Удалить</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="showCreateModal || editingItem" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeModal">
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col transform transition-all">
          <div class="px-6 pt-6 pb-4 border-b border-gray-200 relative flex-shrink-0">
            ...
            <h2 class="text-2xl font-bold text-gray-900 pr-8">{{ editingItem ? 'Редактировать' : 'Добавить фото' }}</h2>
          </div>
...
const formData = ref({ image_url: '', title: '', description: '', text: '', pages: [] as string[] });

const fetchGallery = async () => {
  loading.value = true;
  error.value = '';
  try {
    gallery.value = await adminApi.getGallery();
  } catch (err) {
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await adminApi.updateGalleryItem(editingItem.value.id, formData.value);
    } else {
      await adminApi.createGalleryItem(formData.value);
    }
    closeModal();
    await fetchGallery();
  } catch (err) {
...
const closeModal = () => {
  ...
  formData.value = { image_url: '', title: '', description: '', text: '', pages: [] };
};
